...
class CellRepository {
    ...
    fun getCellsList(onResult:BaseCallback<List<CellModel>>) {
        ApiService.invoke().getCellsList()
            .enqueue(object : Callback<ResponseCell> {
                override fun onFailure(call: Call<ResponseCell>, t: Throwable) {
                    t.message?.let { onResult.onUnsuccessful(it) }
                }

                override fun onResponse(call: Call<ResponseCell>, response: Response<ResponseCell>) {
